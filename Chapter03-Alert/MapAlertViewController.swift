...
class MapAlertViewController: UIViewController {
    ...
    @objc func mapAlert( _ sender: UIButton) {
        ...
        //뷰 컨트롤러에 맵킷 뷰를 추가한다.
        let mapkiView = MKMapView(frame: CGRect(x: 0, y: 0, width: 0, height: 0))
        contentVC.view = mapkiView
        ...
        // 2 지도에서 보여줄 넓이, 일종의 축척, 숫자가 작을수록 범위를 확대시켜서 보여준다.
        ...
        // 4 지도 뷰에 표시
        mapkiView.region = region
        mapkiView.regionThatFits(region)
        
        // 5 위치 를 핀으로 표시
        let point = MKPointAnnotation()
        point.coordinate = pos
        mapkiView.addAnnotation(point)
